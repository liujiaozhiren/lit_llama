...
class MergedLinear(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if r > 0 and any(enable_lora):
            self.w = nn.Parameter(
                self.weight.new_zeros((1, in_features)))  # (1, 128)
    ...
    def train(self, mode: bool = True):
        ...
        # the following process merge linear layer weight with LoRA A*B function,
        # so that when processing forward() we only need to run: return F.linear(x, T(self.weight), bias=self.bias)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.merged:
            ...
            if self.r > 0:
                lora_w = torch.sigmoid(F.linear(x, self.w))  # (64, 64, 128) @ (1, 128) -> (64, 64, 1)
                after_A = F.linear(self.lora_dropout(x), self.lora_A)  # (64, 64, 128) @ (4, 128) -> (64, 64, 4)
                after_A = lora_w * after_A
                # For F.conv1d:
