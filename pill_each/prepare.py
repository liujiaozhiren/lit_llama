...
def gen_sentence(instruction, poi_token, poi_list, max_poi_len, tokenizer, device, max_token_len):
    ...
    pos_input = torch.zeros((len(lang_input), 2), device=device, dtype=torch.float32)
    pos_start_idx, patch_len = len(lang_input), max_poi_len + len(encoded_split)
    ...
    for place_id in places_id:
        ...
        pos_input = torch.cat([pos_input, pos_tensor.repeat(len(encoded_place), 1)])
    ...
    return lang_input, pos_input, pos_start_idx, patch_len, num_poi
...
def prepare(datas: list, poi_list: list, tokenizer, padded_vocab_size=None):
    ...
    for i, data in enumerate(datas):
        ...
        prompt_sentence, spatial_addition, pos_start_idx, patch_len, num = gen_sentence(instruction, poi_seq, poi_list,
                                                                                   max_poi_len, tokenizer, 'cpu', 256)
        # spatial_addition = torch.stack(spatial_addition[:len(prompt_sentence)])
        legal_poi_seq = poi_seq[:num]
        ...
        trainable_mask = torch.zeros(sample['spatial_inputs'].shape[0], dtype=torch.bool)
